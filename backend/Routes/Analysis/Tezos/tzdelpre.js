...
async function getPricesAndMarketCap(fiat) {
	...
	for (i = 0; i < priceAndMarketCapData.length; i++) {
		...
		//slow below? maybe formatDate quicker
...
//REALIZE ROUTE
//REALIZE ROUTE
async function realize(foundRealizeHistory, realizedQuantity) {
    ...
    */
	...
	//doc.font('fonts/PalatinoBold.ttf').fontSize(16).text(100, 100);
	...
	realizedObj = {
		...
		realizingRewardBasisAgg: realizingBasisAgg,
		realizingRewardDepAgg: realizingDepAgg,
		realizingRewardMVDAgg: realizingMVdAgg,
...
async function getRewardsBakers(address) {
    ...
    while (true) {
        try {
            let url = `https://api.tzkt.io/v1/accounts/${address}/operations?type=endorsement,baking,nonce_revelation,double_baking,double_endorsing,transaction,origination,delegation,reveal,revelation_penalty&lastId=${lastId}&limit=10000&sort=0`;
...
module.exports = { realize, autoAnalysis };
